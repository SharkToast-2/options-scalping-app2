...
Identifies mid-cap stocks with high growth potential based on technical indicators
"""
...
class MidCapScreener:
    """Screens mid-cap stocks for growth potential"""
    
    def __init__(self):
        # Mid-cap stocks (market cap between $2B and $10B)
        self.midcap_symbols = [
            # Technology
            'AMD', 'NVDA', 'INTC', 'QCOM', 'AVGO', 'MU', 'KLAC', 'LRCX', 'AMAT', 'ADI',
            'TXN', 'MRVL', 'WDC', 'STX', 'HPQ', 'DELL', 'CRM', 'ORCL', 'ADBE', 'INTU',
            
            # Healthcare
            'GILD', 'BIIB', 'REGN', 'VRTX', 'ALXN', 'ILMN', 'DXCM', 'IDXX', 'ALGN', 'WST',
            'COO', 'TFX', 'HOLX', 'BAX', 'ZBH', 'ABMD', 'ISRG', 'EW', 'ABT', 'TMO',
            
            # Consumer
            'NKE', 'SBUX', 'MCD', 'YUM', 'CMG', 'CHD', 'CLX', 'PG', 'KO', 'PEP',
            'COST', 'TGT', 'HD', 'LOW', 'TJX', 'ROST', 'ULTA', 'NFLX', 'DIS', 'CMCSA',
            
            # Financial
            'GS', 'MS', 'BLK', 'SCHW', 'ICE', 'CME', 'SPGI', 'MCO', 'FIS', 'FISV',
            'V', 'MA', 'AXP', 'COF', 'DFS', 'SYF', 'TFC', 'USB', 'PNC', 'WFC',
            ...
            'LVS', 'WYNN', 'MGM', 'CZR', 'PENN', 'BYD', 'RRR', 'CHDN', 'BALY', 'PENN',
        ...
        self.midcap_symbols = sorted(list(set(self.midcap_symbols)))
    ...
    def screen_midcap_stocks(self, data_fetcher, max_workers: int = 5) -> List[Dict]:
        """Screen mid-cap stocks for growth potential"""
        logger.info(f"🔍 Screening {len(self.midcap_symbols)} mid-cap stocks...")
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            future_to_symbol = {
                ...
                for symbol in self.midcap_symbols
            }
...
# Global instance
midcap_screener = MidCapScreener()

def get_top_midcap_stocks(data_fetcher, max_workers: int = 5) -> List[Dict]:
    """Get top 10 mid-cap stocks with growth potential"""
    return midcap_screener.screen_midcap_stocks(data_fetcher, max_workers)

def get_midcap_stock_analysis(symbol: str, data_fetcher) -> Optional[Dict]:
    """Get detailed analysis for a specific mid-cap stock"""
    return midcap_screener.analyze_stock(symbol, data_fetcher) 